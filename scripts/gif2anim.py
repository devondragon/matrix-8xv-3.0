...
History: 
use numpy instead of ImageMagick. 2021/09/25
generate .sml animation for 8xv 3.0  2022/11/15 [chronicrebellion]
...
def pad_resize(src, dst_size):
    ...
    #updated to LANCZOS due to ANTIALIAS deprecation warning
    scaled_src = src.resize(scale_size, Image.LANCZOS)
...
def process_gif(path, dst_width, dst_height):
    ...
    i = 0
    p = im.getpalette() #no longer used but will keep it here
    ...
    try:
        while True:
            # commenting out due to illegal image mode using putpalette
...
if __name__ == "__main__":
    if (len(sys.argv) < 3):
        print("Usage: gif2anim in_file out_file <anim> | <auxi> | <amft> | <sml>")
    else: 
        if sys.argv[3] not in ["anim", "auxi", "amft", "sml"]:
